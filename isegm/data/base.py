...
class ISDataset(jtorch.utils.data.Dataset):
    def __init__(self,
                 augmentator=None,
        ...
        # self.to_tensor2 = transforms.ToTensor()
    ...
    def __getitem__(self, index):
        ...
        # print((self.to_tensor(sample.image).transpose(2, 0, 1).shape))
        # print((self.to_tensor2(sample.image).numpy().shape))
        # assert(self.to_tensor(sample.image).transpose(2, 0, 1).shape==self.to_tensor2(sample.image).numpy().shape)
        output = {
            'images': jtorch.array(self.to_tensor(sample.image).transpose(2, 0, 1)),
            # 'images': transform.ToTensor(sample.image),
