...
def init_experiment(args, model_name):
    ...
    if cfg.distributed:
        jtorch.distributed.init_process_group(backend='nccl', init_method='env://')
        if args.workers > 0:
            jtorch.multiprocessing.set_start_method('forkserver', force=True)
    ...
    if cfg.distributed:
        cfg.device = jtorch.device('cuda')
        cfg.gpu_ids = [cfg.gpu_ids[cfg.local_rank]]
        jtorch.cuda.set_device(cfg.gpu_ids[0])
    else:
        if cfg.multi_gpu:
            os.environ['CUDA_VISIBLE_DEVICES'] = cfg.gpus
            ngpus = jtorch.cuda.device_count()
            assert ngpus >= cfg.ngpus
        cfg.device = jtorch.device(f'cuda:{cfg.gpu_ids[0]}')
