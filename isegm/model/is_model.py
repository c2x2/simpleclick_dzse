...
class ISModel(nn.Module):
    ...
    def get_coord_features(self, image, prev_mask, points):
        ...
        if prev_mask is not None:
            coord_features = jtorch.cat((prev_mask, coord_features), dim=1)

        return coord_features


def split_points_by_order(tpoints: jtorch.Tensor, groups):
    ...
    group_points = [jtorch.tensor(x, dtype=tpoints.dtype, device=tpoints.device)
