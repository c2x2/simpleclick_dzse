...
def resnet34_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        filtered_orig_dict = _safe_state_dict_filtering(
            jtorch.hub.load(GLUON_RESNET_TORCH_HUB, 'gluon_resnet34_v1b', pretrained=True).state_dict(),
            model_dict.keys()
        )
...
def resnet50_v1s(pretrained=False, **kwargs):
    ...
    if pretrained:
        model_dict = model.state_dict()
        filtered_orig_dict = _safe_state_dict_filtering(
            jtorch.hub.load(GLUON_RESNET_TORCH_HUB, 'gluon_resnet50_v1s', pretrained=True).state_dict(),
            model_dict.keys()
        )
        model_dict.update(filtered_orig_dict)
        model.load_state_dict(model_dict)
...
def resnet101_v1s(pretrained=False, **kwargs):
    ...
    if pretrained:
        model_dict = model.state_dict()
        filtered_orig_dict = _safe_state_dict_filtering(
            jtorch.hub.load(GLUON_RESNET_TORCH_HUB, 'gluon_resnet101_v1s', pretrained=True).state_dict(),
            model_dict.keys()
        )
        model_dict.update(filtered_orig_dict)
        model.load_state_dict(model_dict)
...
def resnet152_v1s(pretrained=False, **kwargs):
    ...
    if pretrained:
        model_dict = model.state_dict()
        filtered_orig_dict = _safe_state_dict_filtering(
            jtorch.hub.load(GLUON_RESNET_TORCH_HUB, 'gluon_resnet152_v1s', pretrained=True).state_dict(),
            model_dict.keys()
        )
        model_dict.update(filtered_orig_dict)
        model.load_state_dict(model_dict)
