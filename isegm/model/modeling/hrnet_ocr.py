...
class HighResolutionNet(nn.Module):
    ...
    def aggregate_hrnet_features(self, x):
        ...
        x1 = nn.interpolate(x[1], size=(x0_h, x0_w),
                           mode='bilinear', align_corners=self.align_corners)
        x2 = nn.interpolate(x[2], size=(x0_h, x0_w),
                           mode='bilinear', align_corners=self.align_corners)
        x3 = nn.interpolate(x[3], size=(x0_h, x0_w),
                           mode='bilinear', align_corners=self.align_corners)

        return jtorch.cat([x[0], x1, x2, x3], 1)
    ...
    def load_pretrained_weights(self, pretrained_path=''):
        ...
        if not os.path.exists(pretrained_path):
            ...
            exit(1)
        pretrained_dict = jtorch.load(pretrained_path, map_location={'cuda:0': 'cpu'})
