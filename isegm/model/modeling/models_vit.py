...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224,224), patch_size=(16, 16), in_chans=3, num_classes=1000, embed_dim=768, 
        ...
        self.cls_token = nn.Parameter(jtorch.zeros(1, 1, embed_dim))
        self.pos_embed = nn.Parameter(jtorch.zeros(1, num_patches + 1, embed_dim)) # learnable positional embedding
    ...
    def init_weights_from_pretrained(self, pretrained_path):
        if pretrained_path:
            checkpoint = jtorch.load(pretrained_path, map_location='cpu')
    ...
    def init_weights(self):
        ...
        jtorch.init.xavier_uniform_(w.view([w.shape[0], -1]))

        # Initialize cls_token and pos_embed with normal distribution
        jtorch.normal(self.cls_token, std=.02)
        jtorch.normal(self.pos_embed, std=.02)
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            jtorch.init.xavier_uniform_(m.weight)
            if m.bias is not None:
                jtorch.init.constant_(m.bias, 0)
        elif isinstance(m, nn.LayerNorm):
            jtorch.init.constant_(m.bias, 0)
            jtorch.init.constant_(m.weight, 1.0)
    ...
    def shuffle(self, x):
        ...
        """
        B, N, C = x.shape
        noise = jtorch.rand(B, N, device=x.device)
        ids_shuffle = jtorch.argsort(noise, dim=1)
        ids_restore = jtorch.argsort(ids_shuffle, dim=1)

        x_shuffle = jtorch.gather(x, 1, index=ids_shuffle.unsqueeze(-1).repeat(1, 1, C))

        return x_shuffle, ids_restore

    def unshuffle(self, x, ids_restore):
        B, N, C = x.shape
        x_unshuffle = jtorch.gather(x, 1, index=ids_restore.unsqueeze(-1).repeat(1, 1, C))
    ...
    def forward(self, x):
        ...
        x = jtorch.cat((cls_token, x), dim=1)
