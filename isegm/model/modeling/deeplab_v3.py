...
class DeepLabV3Plus(nn.Module):
    ...
    def forward(self, x, additional_features=None):
        with ExitStack() as stack:
            if self.inference_mode:
                stack.enter_context(jtorch.no_grad())

            c1, _, c3, c4 = self.backbone(x, additional_features)
            c1 = self.skip_project(c1)

            x = self.aspp(c4)
            x = nn.interpolate(x, c1.size()[2:], mode='bilinear', align_corners=True)
...
class _ASPP(nn.Module):
    ...
    def forward(self, x):
        x = jtorch.cat([block(x) for block in self.concurent], dim=1)
...
class _AsppPooling(nn.Module):
    ...
    def forward(self, x):
        pool = self.gap(x)
        return nn.interpolate(pool, x.size()[2:], mode='bilinear', align_corners=True)
