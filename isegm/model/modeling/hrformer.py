...
class HRT_B_OCR_V3(nn.Module):
    ...
    def forward(self, x_):
        ...
        feat1 = x[0]
        feat2 = nn.interpolate(x[1], size=(h, w), mode="bilinear", align_corners=True)
        feat3 = nn.interpolate(x[2], size=(h, w), mode="bilinear", align_corners=True)
        feat4 = nn.interpolate(x[3], size=(h, w), mode="bilinear", align_corners=True)

        feats = jtorch.cat([feat1, feat2, feat3, feat4], 1)
