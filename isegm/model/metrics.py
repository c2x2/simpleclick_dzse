...
class AdaptiveIoU(TrainMetric):
    ...
    def update(self, pred, gt):
        ...
        gt_mask_area = jtorch.sum(gt_mask, dims=(1, 2)).detach().numpy()
        # gt_mask_area = jtorch.ops.reduce_add_(gt_mask, dims=[1, 2]).detach().numpy()
...
def _compute_iou(pred_mask, gt_mask, ignore_mask=None, keep_ignore=False):
    if ignore_mask is not None:
        pred_mask = jtorch.where(ignore_mask, jtorch.zeros_like(pred_mask), pred_mask)

    reduction_dims = misc.get_dims_with_exclusion(gt_mask.dim(), 0)
    union = jtorch.mean((pred_mask | gt_mask).float(), dims=reduction_dims).detach().cpu().numpy()
    intersection = jtorch.mean((pred_mask & gt_mask).float(), dims=reduction_dims).detach().cpu().numpy()
    nonzero = union > 0
