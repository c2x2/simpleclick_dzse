...
class BRSBasePredictor(BasePredictor):
    ...
    def _get_clicks_maps_nd(self, clicks_lists, image_shape, radius=1):
        ...
        with jtorch.no_grad():
            pos_clicks_map = jtorch.from_numpy(pos_clicks_map).to(self.device)
            neg_clicks_map = jtorch.from_numpy(neg_clicks_map).to(self.device)
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        def get_prediction_logits(scale, bias):
            ...
            if self.insertion_mode == 'after_c4':
                ...
                x = F.interpolate(x, mode='bilinear', size=self._c1_features.size()[2:],
                                  align_corners=True)
                x = jtorch.cat((x, self._c1_features), dim=1)
        ...
        with jtorch.no_grad():
            if self.opt_functor.best_prediction is not None:
                ...
                opt_data_nd = jtorch.from_numpy(self.opt_data).to(self.device)
    ...
    def _get_head_input(self, image_nd, points):
        with jtorch.no_grad():
            ...
            if self.net.rgb_conv is not None:
                x = self.net.rgb_conv(jtorch.cat((image_nd, coord_features), dim=1))
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        if self.opt_data is None or is_image_changed:
            if self.optimize_target == 'dmaps':
                ...
                opt_channels = 3
            bs = image_nd.shape[0] // 2 if self.with_flip else image_nd.shape[0]
            self.opt_data = jtorch.zeros((bs, opt_channels, image_nd.shape[2], image_nd.shape[3]),
                                        device=self.device, dtype=jtorch.float32)
        ...
        def get_prediction_logits(opt_bias):
            ...
            if self.net.rgb_conv is not None:
                x = self.net.rgb_conv(jtorch.cat((input_image, dmaps), dim=1))
        ...
        if num_clicks > self.optimize_after_n_clicks:
            ...
            self.opt_data = jtorch.from_numpy(opt_result[0]).view(self.opt_data.shape).to(self.device)
