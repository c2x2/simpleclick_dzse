...
class BaseOptimizer:
    ...
    def __call__(self, x):
        opt_params = jtorch.from_numpy(x).float().to(self.device)
        opt_params.requires_grad_(True)

        with jtorch.enable_grad():
            opt_vars, reg_loss = self.unpack_opt_params(opt_params)
            result_before_sigmoid = self._get_prediction_logits(*opt_vars)
            result = jtorch.sigmoid(result_before_sigmoid)

            pos_mask, neg_mask = self._click_masks
            if self.with_flip and self.flip_average:
                result, result_flipped = jtorch.chunk(result, 2, dim=0)
                result = 0.5 * (result + jtorch.flip(result_flipped, dims=[3]))
...
class InputOptimizer(BaseOptimizer):
    def unpack_opt_params(self, opt_params):
        ...
        if self.with_flip:
            opt_params_flipped = jtorch.flip(opt_params, dims=[3])
            opt_params = jtorch.cat([opt_params, opt_params_flipped], dim=0)
        reg_loss = self.reg_weight * jtorch.sum(opt_params**2)
...
class ScaleBiasOptimizer(BaseOptimizer):
    ...
    def unpack_opt_params(self, opt_params):
        scale, bias = jtorch.chunk(opt_params, 2, dim=0)
        reg_loss = self.reg_weight * (jtorch.sum(scale**2) + self.reg_bias_weight * jtorch.sum(bias**2))

        if self.scale_act == 'tanh':
            scale = jtorch.tanh(scale)
        elif self.scale_act == 'sin':
            scale = jtorch.sin(scale)
