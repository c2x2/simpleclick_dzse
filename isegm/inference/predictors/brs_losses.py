...
class BRSMaskLoss(jtorch.nn.Module):
    ...
    def forward(self, result, pos_mask, neg_mask):
        ...
        pos_target = jtorch.sum(pos_diff ** 2)
        pos_target = pos_target / (jtorch.sum(pos_mask) + self._eps)

        neg_diff = result * neg_mask
        neg_target = jtorch.sum(neg_diff ** 2)
        neg_target = neg_target / (jtorch.sum(neg_mask) + self._eps)
        
        loss = pos_target + neg_target

        with jtorch.no_grad():
            f_max_pos = jtorch.max(jtorch.abs(pos_diff)).item()
            f_max_neg = jtorch.max(jtorch.abs(neg_diff)).item()

        return loss, f_max_pos, f_max_neg


class OracleMaskLoss(jtorch.nn.Module):
    def __init__(self):
        super().__init__()
        self.gt_mask = None
    ...
    def forward(self, result, pos_mask, neg_mask):
        ...
        if self.predictor.object_roi is not None:
            ...
            gt_mask = jtorch.nn.functional.interpolate(gt_mask, result.size()[2:],  mode='bilinear', align_corners=True)

        if result.shape[0] == 2:
            gt_mask_flipped = jtorch.flip(gt_mask, dims=[3])
            gt_mask = jtorch.cat([gt_mask, gt_mask_flipped], dim=0)
