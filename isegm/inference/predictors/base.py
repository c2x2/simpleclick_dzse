...
class BasePredictor(object):
    ...
    def set_input_image(self, image):
        ...
        image_nd = jtorch.array(image_nd)
        for transform in self.transforms:
            transform.reset()
        self.original_image = image_nd
        if len(self.original_image.shape) == 3:
            self.original_image = self.original_image.unsqueeze(0)
        self.prev_prediction = jtorch.zeros_like(self.original_image[:, :1, :, :])
    ...
    def get_prediction(self, clicker, prev_mask=None):
        ...
        if hasattr(self.net, 'with_prev_mask') and self.net.with_prev_mask:
            input_image = jtorch.cat((input_image, prev_mask), dim=1)
    ...
    def apply_transforms(self, image_nd, clicks_lists):
        ...
        # image_nd = torch.tensor(image_nd.numpy())
        for t in self.transforms:
            image_nd, clicks_lists = t.transform(image_nd, clicks_lists)
            is_image_changed |= t.image_changed
        # image_nd = jtorch.array(image_nd.numpy())
    ...
    def get_points_nd(self, clicks_lists):
        ...
        # return jtorch.array(total_clicks, device=self.device)
        return jtorch.array(total_clicks)
