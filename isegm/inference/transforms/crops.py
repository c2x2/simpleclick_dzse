...
class Crops(BaseTransform):
    ...
    def transform(self, image_nd, clicks_lists: List[List[Click]]):
        ...
        image_crops = jtorch.cat(image_crops, dim=0)
        self._counts = jtorch.tensor(self._counts, device=image_nd.device, dtype=jtorch.float32)
    ...
    def inv_transform(self, prob_map):
        ...
        new_prob_map = jtorch.zeros((1, 1, *self._counts.shape),
        ...
        for dy in self.y_offsets:
            for dx in self.x_offsets:
                ...
                crop_indx += 1
        new_prob_map = jtorch.div(new_prob_map, self._counts)
