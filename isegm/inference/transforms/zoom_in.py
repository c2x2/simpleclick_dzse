...
class ZoomIn(BaseTransform):
    ...
    def inv_transform(self, prob_map):
        ...
        prob_map = jtorch.nn.interpolate(prob_map, size=(rmax - rmin + 1, cmax - cmin + 1),
                                                   mode='bilinear', align_corners=True)

        if self._prev_probs is not None:
            new_prob_map = jtorch.zeros(*self._prev_probs.shape, device=prob_map.device, dtype=prob_map.dtype)
...
def get_roi_image_nd(image_nd, object_roi, target_size):
    ...
    with jtorch.no_grad():
        ...
        roi_image_nd = jtorch.nn.interpolate(roi_image_nd, size=(new_height, new_width),
                                                       mode='bilinear', align_corners=True)
