...
from jtorch.utils.data import DataLoader
...
class ISTrainer(object):
    ...
    def training(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            if self.is_master:
                ...
                # print("get_lr() output:", self.lr_scheduler.get_lr())
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        # with jtorch.set_grad_enabled(not validation):
        if not validation:
            jtorch.enable_grad = True  # 启用梯度计算
        else:
            jtorch.enable_grad = False  # 禁用梯度计算
        batch_data = {k: v.to(self.device) for k, v in batch_data.items()}
        image, gt_mask, points = batch_data['images'], batch_data['instances'], batch_data['points']
        orig_image, orig_gt_mask, orig_points = image.clone(), gt_mask.clone(), points.clone()

        prev_output = jtorch.zeros_like(image, dtype=jtorch.float32)[:, :1, :, :]
        ...
        with jtorch.no_grad():
            ...
            for click_indx in range(num_iters):
                ...
                net_input = jtorch.cat((image, prev_output), dim=1) if self.net.with_prev_mask else image

                prev_output = jtorch.sigmoid(eval_model(net_input, points)['instances'])
            ...
            if self.net.with_prev_mask and self.prev_mask_drop_prob > 0 and last_click_indx is not None:
                ...
                prev_output[zero_mask] = jtorch.zeros_like(prev_output[zero_mask])
    ...
    def save_visualization(self, splitted_batch_data, outputs, global_step, prefix):
        ...
        predicted_instance_masks = jtorch.sigmoid(outputs['instances']).detach().cpu().numpy()
...
def load_weights(model, path_to_weights):
    ...
    new_state_dict = jtorch.load(path_to_weights, map_location='cpu')['state_dict']
